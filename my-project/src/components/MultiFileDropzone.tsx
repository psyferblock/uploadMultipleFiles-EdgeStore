...
import { DbFileType, FileType } from "./MenuPdfSetup";
...
type InputProps = {
	...
	onFieldChange: (something: any) => void;
	finalFormValues: FileType[];
...
const MultiFileDropzone = React.forwardRef<HTMLInputElement, InputProps>(
	(
		...
		if (dropzoneOptions?.maxFiles && values?.length) {
			disabled = disabled ?? values.length >= dropzoneOptions.maxFiles;
		}
		...
		const {
			...
			onDrop: (acceptedFiles) => {
				...
				if (files) {
					...
					void onChange?.([...(values ?? []), ...addedFiles]);
					// onFieldChange(addedFiles.map((file) => file.file.).join(", "))
					onFieldChange(finalFormValues);
				}
			},
			...dropzoneOptions,
		});
		...
		return (
			<div className="w-full">
				<div className="flex w-full flex-col gap-2">
					...
					{/* Selected Files */}
					{values?.map(
						({ file, abortController, progress, key }, i) => (
							<div
								key={i}
								className="flex h-20 w-full flex-col justify-center rounded border border-gray-300 px-4 py-2"
							>
								<div className="flex items-center justify-between gap-10 text-gray-500 dark:text-white">
									<FileIcon
										size="30"
										className="shrink-0"
									/>
